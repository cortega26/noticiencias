...
| (3) Script weight / loading vs LAW-F8                                             | ~135 KB gzipped `gtag.js` alone; GTM + consent platform typically push real deployments past ~285 KB. Heaviest; works against the 150 KB search-budget culture and CWV.                              | ~2.5 KB gzipped single script, deferrable. Negligible CWV impact.                                                                                                             | Lightweight `beacon.min.js` (`type="module"`); RUM-based and reports Core Web Vitals itself — it measures LAW-F8 rather than hurting it.                                                                                                                                                                                      |
...
| (6) Answers ADR-0008's per-path Worker-headroom question?                         | **No.** Client pageview beacons cannot count Worker invocations per path (static assets, `/llm-md/*` internal fetches, bots, non-JS clients are invisible to them).                                  | **No** — same pageview-only limitation.                                                                                                                                       | **Partially — and the only one that can.** The beacon alone is equally pageview-only (say so plainly), BUT Cloudflare already fronts the zone: per-route Worker invocations are available from the same vendor/account via Workers Observability / edge analytics with no second pipeline, no new processor, no extra script. |
...
(no banner, fits `transparencia.md:23` as-is), lightweight under LAW-F8, a
zero-new-processor choice (Cloudflare already fronts the zone and runs the
`/api/*` Worker), and the only one co-located with the data that settles
ADR-0008's deferred per-path Worker-headroom question.
...
vendor; CWV-neutral instrumentation that also reports CWV; Worker-headroom
